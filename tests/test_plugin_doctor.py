...
def test_plugin_doctor_reports_healthy_skill_plugin(tmp_path: Path) -> None:
    plugins_home = tmp_path / ".xiaoman-plugin"
    ...
    (plugin_root / ".xiaoman-plugin").mkdir()
    ...
    sync_result = PluginSkillLinker(
        workspace=workspace,
        plugin_roots=[plugins_home / "cache"],
        memory_engine=None,
    ).sync([plugin])
    materialized = plugin_skill_materialization_path(
        workspace / "skills",
        "demo-skill",
    )
...
def test_plugin_doctor_reports_missing_skill_and_inactive_lifecycle(
    tmp_path: Path,
) -> None:
    plugins_home = tmp_path / ".xiaoman-plugin"
    workspace = tmp_path / "workspace"
