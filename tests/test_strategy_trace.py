...
def test_build_strategy_trace_envelope_uses_subject_scope():
    ...
    assert payload["trace_type"] == "proactive_rate"
    assert payload["source"] == "proactive.loop"
    assert payload["subject"] == {"kind": "global", "id": "rate"}
    assert payload["payload"] == {"mode": "adaptive"}
