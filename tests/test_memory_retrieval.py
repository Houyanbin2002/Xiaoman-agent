...
def test_cosine_ranking_basic(tmp_path):
    """vector_search 按 cosine 相似度降序返回结果。"""
...
def test_superseded_excluded_from_retrieval(tmp_path):
    """superseded 条目不出现在默认检索结果中。"""
    store = MemoryStore2(tmp_path / "m.db")
...
def test_score_threshold_filters_low_matches(tmp_path):
    """score_threshold 过滤低相似度条目。"""
    store = MemoryStore2(tmp_path / "m.db")
...
def test_type_filter_works(tmp_path):
    """memory_types 只返回指定类型。"""
    store = MemoryStore2(tmp_path / "m.db")
