...
覆盖工具发现隔离、会话运行元数据更新和历史消息类型转换；断言面向调用方
可观察行为，不依赖 ContextVar 名称等内部实现细节。
"""
...
# ── tool_search 解锁结果解析 ─────────────────────────────────────────────────
#
# Canonical behavior: parse a tool_search JSON result while preserving the
# provider order used by the active LRU tool-discovery path.
...
def test_unlock_extracts_matched_names():
    ...
    d = _make_discovery()
    result = d.unlock_names_from_result(
