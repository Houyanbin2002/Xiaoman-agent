...
def test_subagent_ignores_repeated_process_output_in_loop_guard():
    tool = _DummyTool("process_output")
    provider = _FakeProvider(
        [
            LLMResponse(content="", tool_calls=[ToolCall("s1", "process_output", {"x": 1})]),
            LLMResponse(content="", tool_calls=[ToolCall("s2", "process_output", {"x": 1})]),
            LLMResponse(content="", tool_calls=[ToolCall("s3", "process_output", {"x": 1})]),
...
def test_subagent_ignores_repeated_process_stop_in_loop_guard():
    tool = _DummyTool("process_stop")
    provider = _FakeProvider(
        [
            LLMResponse(content="", tool_calls=[ToolCall("s1", "process_stop", {"x": 1})]),
            LLMResponse(content="", tool_calls=[ToolCall("s2", "process_stop", {"x": 1})]),
            LLMResponse(content="", tool_calls=[ToolCall("s3", "process_stop", {"x": 1})]),
...
def test_agent_loop_ignores_repeated_process_output_in_loop_guard(tmp_path):
    tool = _DummyTool("process_output")
    provider = _FakeProvider(
        [
            LLMResponse(content="", tool_calls=[ToolCall("c1", "process_output", {"x": 1})]),
            LLMResponse(content="", tool_calls=[ToolCall("c2", "process_output", {"x": 1})]),
            LLMResponse(content="", tool_calls=[ToolCall("c3", "process_output", {"x": 1})]),
            LLMResponse(content="状态已确认", tool_calls=[]),
        ]
    )
    loop = _make_agent_loop(tmp_path, provider, tool)
...
def test_agent_loop_ignores_repeated_process_stop_in_loop_guard(tmp_path):
    tool = _DummyTool("process_stop")
    provider = _FakeProvider(
        [
            LLMResponse(content="", tool_calls=[ToolCall("c1", "process_stop", {"x": 1})]),
            LLMResponse(content="", tool_calls=[ToolCall("c2", "process_stop", {"x": 1})]),
            LLMResponse(content="", tool_calls=[ToolCall("c3", "process_stop", {"x": 1})]),
            LLMResponse(content="任务已停止", tool_calls=[]),
        ]
    )
    loop = _make_agent_loop(tmp_path, provider, tool)
