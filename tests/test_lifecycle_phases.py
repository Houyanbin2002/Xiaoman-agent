...
@pytest.mark.asyncio
async def test_before_turn_memory_context_guard_degrades_without_consolidator():
    bus = EventBus()
    session = _DummySession("telegram:123")
    session.messages = [
    ...
    assert ctx.abort is False
    assert state.extra_metadata["memory_context_guard"] == {
        "status": "degraded",
        "pending": 30,
        "threshold": 30,
        "keep_count": 20,
...
@pytest.mark.asyncio
async def test_before_turn_memory_context_guard_accepts_concurrent_consolidation():
    bus = EventBus()
    session = _DummySession("telegram:123")
    session.messages = [
        {"role": "user", "content": f"u{i}"}
    ...
    class _Consolidator:
        async def trigger_memory_consolidation(
            ...
            *,
            ...
            # A background worker advanced the cursor first.  The foreground
            # call then correctly reports that there was nothing left to do.
            session.last_consolidated = len(session.messages) - 20
            return False
    ...
    assert ctx.abort is False
    assert session.last_consolidated == 10
    assert "memory_context_guard" not in state.extra_metadata
    ctx_store.prepare.assert_awaited_once()


@pytest.mark.asyncio
async def test_before_turn_memory_context_guard_degrades_after_failure():
    bus = EventBus()
    session = _DummySession("telegram:123")
    session.messages = [
        {"role": "user", "content": f"u{i}"}
    ...
    assert ctx.abort is False
    assert state.extra_metadata["memory_context_guard"]["status"] == "degraded"
    assert state.extra_metadata["memory_context_guard"]["error_type"] == "RuntimeError"
    ctx_store.prepare.assert_awaited_once()
