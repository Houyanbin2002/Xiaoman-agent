...
async def test_failed_one_shot_is_retained_without_counting_success(
    tmp_path, mock_push, mock_loop, fixed_now
):
    mock_push.execute = AsyncMock(return_value="发送失败：Dashboard 当前不可用")
    svc = make_service(tmp_path, mock_push, mock_loop, fixed_now)
    job = make_job(
        trigger="after", tier="instant", fire_at=fixed_now - timedelta(seconds=1)
    )
    ...
    retained = svc._jobs[job.id]
    assert retained.enabled is False
    assert retained.run_count == 0
    assert retained.last_status == "failed"
    assert "发送失败" in (retained.last_error or "")
...
def test_pre_start_add_preserves_persisted_jobs(
    tmp_path, mock_push, mock_loop, fixed_now
):
    svc = make_service(tmp_path, mock_push, mock_loop, fixed_now)
    existing = make_job(
    ...
    svc.store.save({existing.id: existing})
    new_job = make_job(
    ...
    recovered = {item.id for item in svc.store.load()}
    assert recovered == {existing.id, new_job.id}
