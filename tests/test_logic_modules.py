...
async def test_memory_optimizer_loop_and_memory_port_cover_paths(tmp_path: Path):
    result = SimpleNamespace(to_dict=lambda: {"expired": 0})
    canonical = MagicMock()
    canonical.optimize.return_value = result
    opt = MemoryOptimizer(canonical)
    await opt.optimize()
    canonical.optimize.assert_called_once_with()
...
def test_session_get_history_bounds_consolidated_tail():
    session = Session("cli:1")
    for i in range(5):
