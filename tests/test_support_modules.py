...
@pytest.mark.asyncio
async def test_tool_registry_context_is_isolated_between_concurrent_turns():
    registry = ToolRegistry()
    registry.register(_DummyTool())
    both_ready = asyncio.Event()
    ready = 0

    async def execute(channel: str, chat_id: str) -> dict[str, object]:
        nonlocal ready
        ...
        result = await registry.execute("dummy", {"name": "ok", "count": 1})
        return json.loads(str(result))

    first, second = await asyncio.gather(
        execute("dashboard", "chat-a"),
        execute("telegram", "chat-b"),
    )

    assert first["channel"] == "dashboard"
    assert first["chat_id"] == "chat-a"
    assert second["channel"] == "telegram"
    assert second["chat_id"] == "chat-b"
...
def test_context_builder_builds_prompt_messages_and_assistant_blocks(
    ...
    assert "Xiaoman 自我认知" not in prompt
    assert "## 环境" not in prompt
    assert "memory block" in context_frame
    assert "Xiaoman 自我认知" in context_frame
    assert "## 环境" in context_frame
    ...
    assert "## 环境" not in messages[0]["content"]
    assert "## Current Session" not in messages[0]["content"]
    assert "## 环境" in messages[-2]["content"]
    assert "## Current Session" in messages[-2]["content"]
    assert messages[-1]["role"] == "user"
    assert len(messages[-1]["content"]) == 3
    stamped_message = messages[-1]["content"][-1]["text"]
    assert stamped_message.startswith("[当前消息时间:")
    assert "[用户上传的附件]" in stamped_message
...
async def test_loop_trigger_and_main_entry_cover_paths(
    ...
    monkeypatch.setattr(
        module,
        "build_app_runtime",
        lambda config, workspace, **kwargs: runtime,
    )
