...
async def test_installed_plugin_shadows_builtin_with_same_name(tmp_path: Path):
    ...
    for plugin_dir in (builtin_plugin, installed_plugin):
        ...
        (plugin_dir / ".xiaoman-plugin").mkdir()
        (plugin_dir / ".xiaoman-plugin" / "plugin.json").write_text(
            json.dumps(
                {
                    "name": "shadow",
                    "version": "0.1.0",
                    "description": "shadow plugin",
                    "xiaoman": {"lifecycle": {"entry": "plugin.py"}},
                }
            ),
            encoding="utf-8",
        )
...
async def test_after_step_tap_hook_fires():
    ...
    hello_mod = next(
        ...
        if k.startswith("xiaoman_plugin_") and k.endswith("_hello")
    )
...
@pytest.mark.asyncio
async def test_loads_installed_xiaoman_plugin_descriptor_without_lifecycle():
    bus = EventBus()
    with tempfile.TemporaryDirectory() as tmp:
        cache_root = Path(tmp) / "cache"
        plugin_root = cache_root / "lab" / "feed" / "0.1.0"
        (plugin_root / ".xiaoman-plugin").mkdir(parents=True)
        ...
        (plugin_root / ".xiaoman-plugin" / "plugin.json").write_text(
            json.dumps(
                {
                    "name": "feed",
                    "version": "0.1.0",
...
async def test_sync_global_registry_covers_builtin_and_installed_plugins(tmp_path: Path):
    ...
    registry_path = mgr.sync_global_registry(plugins_home=tmp_path / ".xiaoman-plugin")
...
@pytest.mark.asyncio
async def test_failed_plugin_initialization_rolls_back_handlers_and_module(
    tmp_path: Path,
):
    plugin_dir = tmp_path / "broken_init"
    plugin_dir.mkdir()
    (plugin_dir / "plugin.py").write_text(
        """
from agent.plugins import Plugin, on_before_turn

class BrokenInit(Plugin):
    name = "broken-init"

    @on_before_turn()
    async def touch(self, event):
        event.extra_metadata["leaked_handler"] = True
        return event
...
""".strip(),
    ...
    bus = EventBus()
    manager = _make_manager([tmp_path], event_bus=bus, tools=ToolRegistry())
    discovered = manager.discover()

    await manager.load_all()
    event = _before_turn_ctx()
    await bus.emit(event)

    module_name = discovered[0]["import_path"]
    assert manager.loaded_count == 0
    assert "leaked_handler" not in event.extra_metadata
    assert module_name not in sys.modules
    assert plugin_registry.get_instance(module_name) is None
    assert plugin_registry.get_handlers_by_module_path(module_name) == []


@pytest.mark.asyncio
async def test_failed_plugin_import_cleans_decorator_metadata_and_module(
    tmp_path: Path,
):
    plugin_dir = tmp_path / "broken_import"
    plugin_dir.mkdir()
    (plugin_dir / "plugin.py").write_text(
        """
from agent.plugins import Plugin, on_before_turn

class BrokenImport(Plugin):
    name = "broken-import"
...
""".strip(),
    ...
    manager = _make_manager([tmp_path], event_bus=EventBus())
    discovered = manager.discover()
...
@pytest.mark.asyncio
async def test_core_runtime_start_wires_plugin_tool_hooks_to_task_executor():
    ...
    task_tool = SimpleNamespace(
        runtime=SimpleNamespace(subagent_executor=subagent_executor)
    )
    loop = FakeLoop()
    ...
    runtime = CoreRuntime(
        ...
        tools=SimpleNamespace(
            get_tool=lambda name: task_tool if name == "task_create" else None
    ...
    assert subagent_executor.received_hooks == plugin_manager.tool_hooks
