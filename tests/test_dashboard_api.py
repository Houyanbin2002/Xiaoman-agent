...
def test_dashboard_binding_requires_explicit_remote_opt_in() -> None:
    _validate_dashboard_binding("127.0.0.1", allow_remote=False)
    _validate_dashboard_binding("::1", allow_remote=False)
    _validate_dashboard_binding("localhost", allow_remote=False)

    with pytest.raises(ValueError, match="只允许监听回环地址"):
        _validate_dashboard_binding("0.0.0.0", allow_remote=False)

    _validate_dashboard_binding("0.0.0.0", allow_remote=True)
...
def test_list_sessions_with_filters(tmp_path) -> None:
    ...
    with TestClient(create_dashboard_app(tmp_path)) as client:
        ...
        assert payload["items"][0]["title"] == "alpha room"
        ...
        assert cli_resp.json()["items"][0]["title"] == "beta room"

        dashboard_resp = client.get(
            "/api/dashboard/sessions",
            params={"channel": "dashboard"},
        )
        assert dashboard_resp.status_code == 200
        assert dashboard_resp.json()["items"][0]["title"] == "请帮我制定本周训练计划"
...
def test_list_update_and_batch_delete_messages(tmp_path) -> None:
    ...
    with TestClient(create_dashboard_app(tmp_path)) as client:
        ...
        chronological_items = chronological.json()["items"]
        assert [item["seq"] for item in chronological_items] == sorted(
            item["seq"] for item in chronological_items
        )
