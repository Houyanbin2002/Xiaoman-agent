...
def test_behavior_rules_keep_one_off_approvals_out_of_task_center():
    prompt = build_agent_behavior_rules_prompt(workspace=Path("."))
    assert "单次工具调用即使需要权限确认，也仍然直接调用该工具" in prompt
    assert "不要仅仅为了 `write_file`" in prompt
    assert "不要把一次性操作包装成持久化任务" in prompt
