...
import infra.providers.llm_provider as provider_module
from agent.mcp.registry import McpServerRegistry
from agent.plugins.manager import ActivePluginInfo
from core.llm import ContextLengthError, ContentSafetyError
...
async def test_provider_chat_and_retry_paths(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("infra.providers.llm_provider.asyncio.sleep", _sleep)
...
@pytest.mark.asyncio
async def test_app_runtime_start_passes_governed_memory_to_optimizer(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
):
    engine = MagicMock(name="engine")
    markdown_store = MagicMock(name="markdown_store")
    governed_memory = MagicMock(name="governed_memory")
    memory_runtime = SimpleNamespace(
        engine=engine,
        markdown=SimpleNamespace(store=markdown_store),
        long_term=governed_memory,
        aclose=AsyncMock(),
    )
    ...
    assert build_memory_optimizer_task.call_args.kwargs["canonical"] is governed_memory
    assert app.dashboard_server.manual_memory_optimizer is memory_optimizer


@pytest.mark.asyncio
async def test_cli_paths(
    ...
    _print_banner()
    assert "xiaoman Agent CLI" in capsys.readouterr().out
...
def test_bootstrap_proactive_builders_cover_enabled_and_disabled_paths(
    ...
    monkeypatch.setattr(
        ...
        lambda canonical: SimpleNamespace(canonical=canonical),
    )
    monkeypatch.setattr(
