...
def test_config_load_reads_prompt_cache_breakpoint_policy(tmp_path: Path):
    cfg_path = tmp_path / "config.toml"
    _write_toml(
        cfg_path,
        {
    ...
    assert cfg.prompt_cache.keep_recent_tool_rounds == 4
    assert cfg.prompt_cache.cold_tool_result_chars == 900
...
""".strip() + "\n",
    ...
    if sys.platform == "win32":
        assert cfg.channels.socket != "/tmp/toml-xiaoman.sock"
...
def test_memory_plugin_resolver_loads_plugin_directory(monkeypatch, tmp_path: Path):
    ...
    import bootstrap.memory_plugins as memory_plugins

    monkeypatch.setattr(memory_plugins, "_PROJECT_ROOT", tmp_path)
...
def test_build_registered_tools_without_mcp_toolset_still_returns_empty_registry(
    ...
    assert runtime.mcp_registry.list_servers() == "当前没有已注册的 MCP server。"
