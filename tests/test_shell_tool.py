...
def test_shell_tool_describes_the_actual_platform_shell():
    tool = ShellTool()
    if os.name == "nt":
        assert "cmd.exe" in tool.description
        assert "Windows cmd.exe" in tool.parameters["properties"]["command"]["description"]
        assert "在 bash 中" not in tool.description
    else:
        assert "POSIX shell" in tool.description
        assert "POSIX shell" in tool.parameters["properties"]["command"]["description"]
...
@pytest.mark.asyncio
async def test_process_output_returns_log_content(monkeypatch, tmp_path):
    """process_output 能读取后台任务已写入的日志内容。"""
    import agent.tools.shell as shell_mod
...
@pytest.mark.asyncio
async def test_process_output_returns_timing_fields(tmp_path):
    """process_output 应返回 elapsed_ms 和 since_last_output_ms。"""
    import agent.tools.shell as shell_mod
...
@pytest.mark.asyncio
async def test_process_output_not_found():
    """process_output 查询不存在的 task_id 返回 error。"""
    tool = ShellTaskOutputTool()
...
@pytest.mark.asyncio
async def test_process_stop_kills_and_removes(monkeypatch):
    """process_stop 发送 SIGKILL 并从注册表移除。"""
    import agent.tools.shell as shell_mod
...
@pytest.mark.asyncio
async def test_process_stop_deletes_log_file(monkeypatch, tmp_path):
    """process_stop 应立即删除日志文件，不依赖 done callback 的延迟清理。"""
    import agent.tools.shell as shell_mod
    ...
    assert not log_path.exists(), "process_stop 后日志文件应已被立即删除"


@pytest.mark.asyncio
async def test_process_output_ttl_deletes_log_file(monkeypatch, tmp_path):
...
@pytest.mark.asyncio
async def test_process_stop_not_found():
    """process_stop 对不存在的任务返回 not_found。"""
    tool = ShellTaskStopTool()
...
@pytest.mark.asyncio
async def test_process_output_ttl_does_not_kill_running_task_without_timeout(
...
@pytest.mark.asyncio
async def test_bg_pump_cancellation_stops_nested_stream_drains(tmp_path):
    import agent.tools.shell as shell_mod

    class _CancellablePipe:
    ...
    class _NeverExits:
        ...
        def __init__(self) -> None:
            self.stdout = _CancellablePipe()
            self.stderr = _CancellablePipe()

        async def wait(self):
            await asyncio.Future()

    proc = _NeverExits()
    log_path = str(tmp_path / "cancelled-pump.log")
    bg = shell_mod._BackgroundTask(
        proc=proc,
        log_path=log_path,
        pump_task=None,
    ...
    pump = asyncio.create_task(shell_mod._bg_pump(proc, log_path, bg))
    await asyncio.sleep(0)
    pump.cancel()
    await asyncio.gather(pump, return_exceptions=True)

    assert proc.stdout.cancelled is True
    assert proc.stderr.cancelled is True
...
@pytest.mark.asyncio
async def test_process_output_timeout_expired(monkeypatch, tmp_path):
    import agent.tools.shell as shell_mod
...
@pytest.mark.asyncio
async def test_process_output_clamps_block_timeout(monkeypatch, tmp_path):
...
@pytest.mark.asyncio
async def test_process_output_handles_internal_timeout_cancellation(
    monkeypatch, tmp_path
):
    import agent.tools.shell as shell_mod
