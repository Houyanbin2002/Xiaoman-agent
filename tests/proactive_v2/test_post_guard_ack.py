...
@pytest.mark.asyncio
async def test_ack_post_guard_fail_alert_cited_stays_pending():
    """post-guard 失败表示没有送达，alert 必须留给后续 tick 重评。"""
    ctx = AgentTickContext()
    ctx.fetched_alerts = [{"id": "a1", "ack_server": "alert-mcp"}]
    ctx.cited_item_ids = ["alert-mcp:a1"]
    ctx.interesting_item_ids = {"alert-mcp:a1"}
    ...
    assert alert_sink.keys == []
    assert sink.not_acked("alert-mcp:a1")


@pytest.mark.asyncio
async def test_ack_post_guard_fail_alert_has_no_regular_ack_fallback():
    """没有 alert_ack_fn 时也不能把未送达 alert 当成已处理。"""
    ctx = AgentTickContext()
    ctx.fetched_alerts = [{"id": "a1", "ack_server": "alert-mcp"}]
    ctx.cited_item_ids = ["alert-mcp:a1"]
    ctx.interesting_item_ids = {"alert-mcp:a1"}
...
async def test_ack_post_guard_fail_content_cited_and_alert_cited_separate_channels():
    """post-guard 失败：content 短 ACK，alert 保持 pending。"""
    ctx = AgentTickContext()
    ctx.fetched_alerts = [{"id": "a1", "ack_server": "alert-mcp"}]
    ctx.fetched_contents = [{"id": "c1", "ack_server": "feed-mcp"}]
...
@pytest.mark.asyncio
async def test_ack_post_guard_fail_uncited_fetched_alerts_stay_pending():
    """未引用的 alert 同样没有送达，不得被清空。"""
    ctx = AgentTickContext()
    ctx.fetched_alerts = [
    ...
    assert alert_sink.keys == []
    assert sink.not_acked("alert-mcp:a1")
    assert sink.not_acked("alert-mcp:a2")
