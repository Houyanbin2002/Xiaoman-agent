...
@pytest.mark.asyncio
async def test_runtime_admission_serializes_same_session_only():
    loop = object.__new__(AgentLoop)
    loop._passive_runtime_locks = {}
    events: list[str] = []
...
@pytest.mark.asyncio
async def test_runtime_admission_allows_different_sessions_in_parallel():
    loop = object.__new__(AgentLoop)
    loop._passive_runtime_locks = {}
    ...
    async def _process(
        ...
        return OutboundMessage(channel=msg.channel, chat_id=msg.chat_id, content=key)

    loop._process = _process
    first = InboundMessage(channel="cli", sender="u", chat_id="1", content="a")
    second = InboundMessage(channel="cli", sender="u", chat_id="2", content="b")

    results = await asyncio.gather(
        AgentLoop._process_with_runtime_admission(loop, first),
        AgentLoop._process_with_runtime_admission(loop, second),
    )

    assert {result.content for result in results} == {"cli:1", "cli:2"}


@pytest.mark.asyncio
async def test_bus_turns_run_different_gateway_sessions_in_parallel():
    loop = object.__new__(AgentLoop)
    loop._passive_runtime_locks = {}
    loop._inbound_capacity = asyncio.Semaphore(4)
    loop._active_tasks = {}
    ...
    async def _process(msg: InboundMessage, **_kwargs: Any) -> OutboundMessage:
    ...
    await asyncio.gather(
        AgentLoop._run_inbound_turn(
            loop,
            InboundMessage(channel="qqbot", sender="u", chat_id="c2c:1", content="qq"),
        ),
        AgentLoop._run_inbound_turn(
            loop,
            InboundMessage(channel="weixin", sender="u", chat_id="dm:2", content="微信"),
        ),
    )

    assert set(started) == {"qqbot:c2c:1", "weixin:dm:2"}
    assert loop.bus.complete_inbound.await_count == 2


@pytest.mark.asyncio
async def test_process_direct_registers_interrupt_and_persists_cancelled_turn():
    loop = object.__new__(AgentLoop)
    loop._passive_runtime_locks = {}
    loop._active_tasks = {}
    loop._active_turn_states = {}
    ...
    async def _process(*_args: Any, **_kwargs: Any) -> OutboundMessage:
    ...
    session = SimpleNamespace(messages=messages, add_message=_add_message)
    session_manager = SimpleNamespace(
        get_or_create=MagicMock(return_value=session),
        append_messages=AsyncMock(return_value=None),
    )
    loop._session_services = SimpleNamespace(session_manager=session_manager)
    task = asyncio.create_task(
        AgentLoop.process_direct(
            loop,
            content="不要再继续",
            session_key="dashboard:stop-test",
            channel="dashboard",
            chat_id="stop-test",
        )
    )
    ...
    result = AgentLoop.request_interrupt(loop, "dashboard:stop-test")

    assert result.status == "interrupted"
    with pytest.raises(asyncio.CancelledError):
        await task
    assert messages[0]["content"] == "不要再继续"
    ...
    assert "dashboard:stop-test" not in loop._active_tasks
