...
const localEsbuildEntrypoint = join(
  projectRoot,
  "node_modules",
  "esbuild",
  "bin",
...
function resolveEsbuildCommand() {
  // Execute the package entrypoint through the current Node binary. Recent Node
  // versions reject direct spawn of .cmd shims on Windows with EINVAL.
  if (existsSync(localEsbuildEntrypoint)) {
    return [process.execPath, localEsbuildEntrypoint];
  }
