...
def _run_compose(paths: ProbePaths, args: list[str]) -> None:
    env = {"XIAOMAN_DEBUG_PROFILE": paths.profile}
    _ = subprocess.run(
...
async def _run_probe(args: argparse.Namespace) -> None:
    ...
    try:
        if args.reset_workspace:
            _run_compose(
                paths,
                ["run", "--rm", "xiaoman-debug", "reset-workspace"],
            )
        if args.start_agent:
            proc = subprocess.Popen(
                ...
                env={
                    **dict(os.environ),
                    "XIAOMAN_DEBUG_PROFILE": paths.profile,
                },
...
def _parse_args() -> argparse.Namespace:
    ...
    _ = parser.add_argument(
        "--disable-qq",
        action="store_true",
        help="运行期间临时给 [plugins.qqbot] 加 enabled=false，结束后恢复。",
    )
