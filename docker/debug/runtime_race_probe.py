...
class RaceHarness:
    def __init__(
        ...
        *,
        ...
        self.timeout = timeout
        self._tmpdir = TemporaryDirectory(prefix="xiaoman-race-")
...
async def scenario_agent_loop_runtime(harness: RaceHarness) -> None:
    ...
    if config.channels.telegram is not None or config.plugins.get("qqbot", {}).get("app_id"):
...
def _parse_args() -> argparse.Namespace:
    ...
    _ = parser.add_argument(
        "--scenario",
        default=os.environ.get("XIAOMAN_RACE_SCENARIO", "all"),
        choices=["all", *SCENARIOS.keys()],
    )
    _ = parser.add_argument(
        "--timeout",
        type=float,
        default=float(os.environ.get("XIAOMAN_RACE_TIMEOUT", "2")),
    )
    _ = parser.add_argument(
        "--trace",
        type=Path,
        default=(
            Path(os.environ["XIAOMAN_RACE_TRACE"])
            if os.environ.get("XIAOMAN_RACE_TRACE")
            else None
        ),
    )
    _ = parser.add_argument(
        ...
        type=Path,
        default=(
            Path(os.environ["XIAOMAN_RACE_CONFIG"])
            if os.environ.get("XIAOMAN_RACE_CONFIG")
            else None
        ),
    )
    _ = parser.add_argument(
        ...
        type=Path,
        default=(
            Path(os.environ["XIAOMAN_RACE_WORKSPACE"])
            if os.environ.get("XIAOMAN_RACE_WORKSPACE")
            else None
        ),
    )
