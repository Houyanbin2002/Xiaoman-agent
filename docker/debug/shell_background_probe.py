...
验证新版 shell 后台设计：agent 能自主发现卡死任务并调用 process_stop，
整个生命周期在单次 turn 内完成，不依赖系统自动回调。

被测行为：
  1. agent 对外观无害的命令启动 shell
  2. 命令 15s 后 auto-promote 转后台，agent 拿到 task_id
  3. agent 用 process_output 轮询，发现从无输出（since_last_output_ms=null）
  4. agent 判定卡死，调用 process_stop
...
class ProbePaths:
    ...
    def socket(self) -> Path:
        return self.profile_dir / "xiaoman.sock"
...
def _run_compose(paths: ProbePaths, args: list[str]) -> None:
    _ = subprocess.run(
        ...
        cwd=paths.repo,
        env={**dict(os.environ), "XIAOMAN_DEBUG_PROFILE": paths.profile},
        check=True,
    )
...
def _build_checks(
    *,
    ...
    process_output_idx = next(
        (i for i, n in enumerate(call_names) if n == "process_output"), -1
    )
    process_stop_idx = next(
        (i for i, n in enumerate(call_names) if n == "process_stop"), -1
    )
    ...
    # process_output 调用里有没有拿到 status=done（不应该，死循环不会自然结束）
    process_output_done = any(
        ...
        for c in all_calls
        if c.get("name") == "process_output"
    )

    checks: dict[str, Any] = {
        # 核心行为验证
        "shell_called": shell_idx >= 0,
        "process_output_called": process_output_idx >= 0,
        "process_stop_called": process_stop_idx >= 0,
        # 顺序：shell → process_output → process_stop
        "correct_call_order": (
            shell_idx >= 0
            and process_output_idx > shell_idx
            and process_stop_idx > process_output_idx
        ),
        ...
        "task_not_done_naturally": not process_output_done,
        # 调用序列摘要（供报告查看）
...
def _write_report(*, report_base: Path, payload: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "",
        "验证 agent 能自主发现卡死的后台 shell 任务并调用 process_stop，",
        "整个生命周期在单次 turn 内完成，不依赖系统自动回调。",
        "",
        "```",
        "shell(死循环命令)",
        "  └─ auto-promote 15s → task_id",
        "       └─ process_output(block=true) → since_last_output_ms=null",
        "            └─ process_stop → 最终回复（单 turn 完成）",
        "```",
        "",
...
# 发给 agent 的 prompt：不暴露循环意图，agent 只能通过 process_output 观察到什么都没输出
_PROBE_PROMPT = (
...
async def _run_probe(args: argparse.Namespace) -> None:
    ...
    try:
        if args.reset_workspace:
            _run_compose(paths, ["run", "--rm", "xiaoman-debug", "reset-workspace"])
        ...
        try:
            ...
            # agent 需要：15s auto-promote + 至少一次 process_output + process_stop + 回复
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="测试 agent 能否自主发现卡死后台 shell 任务并调用 process_stop。"
    )
