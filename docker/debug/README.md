...
这个目录用于临时调试真实入口，例如 Telegram 图片、多模态链路、独立 bot 配置。调试容器基于 Arch Linux，沙盒不会挂载宿主机 `HOME`，也不会挂载正式 `~/.xiaoman/workspace`。
...
```bash
docker compose -f docker/debug/docker-compose.yml run --rm xiaoman-debug setup
```
...
```bash
docker compose -f docker/debug/docker-compose.yml up xiaoman-debug
```
...
```bash
XIAOMAN_DEBUG_PROFILE=multimodal docker compose -f docker/debug/docker-compose.yml run --rm xiaoman-debug setup
XIAOMAN_DEBUG_PROFILE=multimodal docker compose -f docker/debug/docker-compose.yml up xiaoman-debug
```
...
```bash
docker compose -f docker/debug/docker-compose.yml run --rm xiaoman-debug cli
```

CLI socket 固定为 `/sandbox/xiaoman.sock`，不会连接正式实例。

## 打开调试 Dashboard

```bash
docker compose -f docker/debug/docker-compose.yml run --rm --service-ports xiaoman-debug dashboard
```

宿主机访问 `http://127.0.0.1:2237`。调试容器会显式允许容器内的非回环
监听，但 Compose 只把端口发布到宿主机回环地址，避免未鉴权管理接口暴露到局域网。
...
```bash
docker compose -f docker/debug/docker-compose.yml run --rm xiaoman-debug reset-workspace
```
...
`--disable-qq` 会在运行期间临时给当前 profile 的 `[plugins.qqbot]` 加 `enabled = false`，结束后恢复原配置，适合只测 CLI 但该 profile 配了 QQ 官方机器人时使用。
...
```bash
docker compose -f docker/debug/docker-compose.yml run --rm xiaoman-debug \
...
```text
XIAOMAN_RACE_SCENARIO  选择单个场景，默认 all
XIAOMAN_RACE_TIMEOUT   每个等待点的超时秒数，默认 2
XIAOMAN_RACE_TRACE     写出 JSON 结果的路径
XIAOMAN_RACE_CONFIG    指定 config.toml；不指定时生成无外部 channel 的最小配置
XIAOMAN_RACE_WORKSPACE 指定临时 workspace；不指定时使用临时目录
```
