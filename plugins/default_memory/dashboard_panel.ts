/// <reference path="../../types/xiaoman-dashboard.d.ts" />
...
function dmemRenderDeleteBtn(_value: unknown, item: Record<string, unknown>): string {
  ...
  return `<button class="icon-btn row-delete-btn" type="button" onclick="event.stopPropagation();void(async()=>{try{await api('/api/dashboard/memories/batch-delete',{method:'POST',body:JSON.stringify({ids:['${id}']})});window.dispatchEvent(new CustomEvent('xiaoman-dashboard-refresh'))}catch(e){alert(e.message||String(e))}})()" title="删除此条">✕</button>`;
}

async function dmemGetCount(): Promise<number | null> {
  try {
    const info = await api<{ name: string }>("/api/dashboard/memory/engine-info");
    if (info.name !== "default" && info.name !== "xiaoman") return null;
...
window.XiaomanDashboard.registerPlugin({
  id: "default_memory",
  label: "Memory",
  viewLabel: "memory",
  pageSize: 25,
