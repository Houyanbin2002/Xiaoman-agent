...
python scripts/build_akasha_db.py \
  --config config.toml \
  --sessions-db ~/.xiaoman/workspace/sessions.db \
  --db-path    ~/.xiaoman/workspace/memory/akasha.db
```

（不带参数时用默认路径：`~/.xiaoman/workspace/{sessions.db, memory/akasha.db}`、`config.toml`。）
...
store = AkashaStore("~/.xiaoman/workspace/memory/akasha.db")

src = sqlite3.connect("~/.xiaoman/workspace/sessions.db"); src.row_factory = sqlite3.Row
rows = src.execute(
