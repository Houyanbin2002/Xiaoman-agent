...
def plugin_enabled(app: FastAPI) -> bool:
    return _active_memory_engine(app) in {"akasha", "xiaoman"}
...
class AkashaGraphReader:
    ...
    def rebuild_global_graph(self) -> dict[str, Any]:
        ...
        with self._rebuild_lock:
            if (
                ...
                and self._rebuild_thread is not threading.current_thread()
            ):
    ...
    def _build_query_payload(
        ...
        *,
        ...
        colors, legend = community_legend(
            graph, comms, node_to_comm, nodes_by_key, texts
        )
            ...
            payload_edges.append(
                {
                    ...
                    "sim": dense_sim(
                        nodes_by_key[str(src)].get("embedding"),
                        nodes_by_key[str(dst)].get("embedding"),
                    ),
                }
            )
...
def register(app: FastAPI, plugin_dir: Path, workspace: Path) -> list[object]:
    ...
    akasha_db_path = resolve_akasha_db_path(
        workspace=workspace, akasha_config=akasha_config
    )
    ...
    def get_akasha_graph_query(query_id: str) -> dict[str, Any]:
        try:
            ...
            raise HTTPException(
                status_code=404, detail="Akasha 检索记录不存在"
            ) from None
...
def _merge_undirected_edges(rows: list[sqlite3.Row]) -> list[EdgeRow]:
    ...
    return sorted(
        merged.values(), key=lambda item: (item.co_count, item.weight), reverse=True
    )
