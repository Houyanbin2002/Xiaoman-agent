...
class AkashaStore:
    ...
    def list_items_for_dashboard(
        ...
        *,
        q: str = "",
        matching_anchor_ids: list[str] | None = None,
        page: int = 1,
        page_size: int = 50,
        ...
        params: list[object] = []
        if q.strip():
            clauses = ["key LIKE ?", "anchor_id LIKE ?"]
            like = f"%{q.strip()}%"
            params.extend([like, like])
            clean_anchor_ids = [
                item.strip()
                for item in (matching_anchor_ids or [])[:700]
                if item.strip()
            ]
            if clean_anchor_ids:
                placeholders = ",".join("?" for _ in clean_anchor_ids)
                clauses.append(f"anchor_id IN ({placeholders})")
                params.extend(clean_anchor_ids)
