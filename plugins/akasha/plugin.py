...
def _is_memory_engine(engine: object, name: str) -> bool:
    ...
    if str(getattr(description, "name", "")) == name:
        return True
    notes = getattr(description, "notes", {})
    components = notes.get("components", []) if isinstance(notes, dict) else []
    return name == "akasha" and "akasha_episodic" in components
