...
class AkashaMemoryEngine:
    ...
    def reconfigure_embedding(
        ...
        *,
        ...
        """Switch semantic encoding for future writes and recalls in-place."""
    ...
    def list_items_for_dashboard(
        ...
        *,
        ...
        items, total = self._store.list_items_for_dashboard(
            q=q,
            matching_anchor_ids=matching_anchor_ids,
            page=page,
            page_size=page_size,
            sort_by=sort_by,
            sort_order=sort_order,
        )
        return _attach_message_content(items, self._session_db_path), total
...
def _find_matching_message_ids(db_path: Path, query: str) -> list[str]:
...
def _attach_message_content(
    items: list[dict[str, object]],
    db_path: Path,
) -> list[dict[str, object]]:
    anchors = [str(item.get("source_ref") or "") for item in items]
    clean_anchors = [item for item in anchors if item]
    if not clean_anchors or not db_path.exists():
        return items
    placeholders = ",".join("?" for _ in clean_anchors)
    try:
        with sqlite3.connect(str(db_path), timeout=3.0) as db:
            rows = db.execute(
                f"SELECT id, content, ts FROM messages WHERE id IN ({placeholders})",
                clean_anchors,
            ).fetchall()
    except sqlite3.Error:
        return items
    messages = {
        str(message_id): (str(content or "").strip(), str(ts or ""))
        for message_id, content, ts in rows
    }
    enriched: list[dict[str, object]] = []
    for item in items:
        source_ref = str(item.get("source_ref") or "")
        content, timestamp = messages.get(source_ref, ("", ""))
        if not content:
        ...
        extra["turn_key"] = str(item.get("id") or "")
        enriched.append(
            {
                **item,
                "summary": content,
                "content": content,
                "happened_at": timestamp or item.get("happened_at") or "",
                "extra_json": extra,
            }
        )
    return enriched
