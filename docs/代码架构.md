...
- Lifecycle hooks and internal dashboard diagnostic APIs.
...
| Dashboard product page | `bootstrap/dashboard_management/` API + `frontend/dashboard/src/features/` |
| Plugin diagnostics | plugin `dashboard.py` API, rendered by an existing host page when needed |
...
Entity payloads cross the service boundary as normalized mappings. Entity-specific
canonicalization belongs to `core/personal/normalization.py` and the service layer;
do not add a parallel dataclass for every entity unless a production consumer
actually requires that typed contract.
...
`TurnResult` side effects implement the `TurnSideEffect` protocol and are executed
by the orchestrator before or after delivery according to their declared list.
...
- Provider responses are decoded at the adapter boundary into validated completion,
  usage and tool-call views; SDK objects and vendor-specific dictionaries do not
  flow into the Agent runtime.

Configuration and runtime orchestration follow the same direction:

- TOML decoding and validation: `agent/config.py`
- Agent execution/context ports: `agent/core/contracts.py`
- Tool catalog models: `agent/tools/catalog_models.py`; search backends and the
  registry depend on this shared contract instead of importing each other.
- Conversation semantic runtime and batching: `agent/conversation_semantics/`
- Conversation semantic models, evidence and durable batch contract: `core/conversation_semantics/`
- Markdown support-file implementation: `infra/persistence/markdown_memory_store.py`
- Markdown memory ports and maintenance contract: `core/memory/markdown.py`
- Memory retrieval hit contract: `memory2/models.py`; vector search, keyword
  search, RRF fusion and prompt injection exchange the same `MemoryHit` shape.
- Memory plugin application contract: `agent/plugins/memory.py`
- Memory plugin discovery and loading: `bootstrap/memory_plugins.py`; runtime
  memory construction no longer imports the wider Bootstrap wiring registry.
- Disabled memory domain implementation: `core/memory/disabled.py`
- Provider-neutral scheduling models: `core/scheduling/`
- Scheduler execution and persistence orchestration: `agent/scheduler.py`
...
- Personal record events and reconciliation: `core/personal/events.py` and
  `core/personal/memory_reconciliation.py`
...
Dashboard 页面统一由主仓 React 应用构建。插件可以通过 `dashboard.py` 注册诊断
API，但不在运行时编译或注入任意前端面板；需要新增可视化页面时，应在主仓建立
明确的路由、类型和权限边界。`main.tsx` 只保留应用外壳，各功能页通过
`React.lazy` 按当前视图加载；禁止重新建立同步导出全部功能页的聚合入口。
`npm run build` 会在 Vite 构建后执行 Dashboard 包体门禁，入口或任一 JS 分包不得
超过 300KB。
