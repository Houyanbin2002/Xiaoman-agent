...
_TOOL_ALIASES: dict[str, frozenset[str]] = {
    ...
    "recent_history": frozenset(
        {"fetch_messages", "search_messages", "recent_history"}
    ),
...
def _generated_rubric(case: EvalCase) -> tuple[RubricCriterion, ...]:
    ...
    if "required_tool_status" in expected:
        criteria.append(
            RubricCriterion(
                "required_tool_status",
                "要求的工具实际成功并返回有效执行证据",
                weight=2.0,
                hard=bool(expected.get("required_tool_status_hard", True)),
                check="required_tool_status",
            )
        )
    ...
    if "status" in expected:
        criteria.append(
            RubricCriterion(
                "status", "执行状态正确", weight=1.0, hard=True, check="status"
            )
        )
    if "objective_outcome" in expected:
        criteria.append(
            RubricCriterion(
                "objective_outcome",
                "用户目标已真正完成，而非仅正常终止",
    ...
    if "max_latency_ms" in expected:
        criteria.append(
            RubricCriterion(
                "latency", "执行延迟不超过预算", weight=0.5, check="latency"
            )
        )
...
def canonical_rubric(case: EvalCase) -> tuple[RubricCriterion, ...]:
    ...
    covered.update(
        criterion.check
        for criterion in explicit
        if criterion.check and criterion.evaluator == "deterministic"
    )
    ...
    for name, spec in case.expected.get("evidence", {}).items():
        if name in covered:
            raise ValueError(f"evidence id collides with rubric: {name}")
        explicit.append(
            RubricCriterion(
                name,
                f"核验 {name}",
...
def _contains_all(text: str, values: list[Any]) -> tuple[float, str]:
    ...
    return (
        ...
        f"matched {len(found)}/{len(wanted)} required phrases",
    )
...
def _deep_contains(actual: Any, expected: Any, path: str = "state") -> tuple[bool, str]:
    ...
    if isinstance(expected, list):
        ...
        return (
            not missing,
            f"missing values in {path}: {missing}" if missing else f"matched {path}",
        )
    return actual == expected, f"{path} expected {expected!r}, got {actual!r}"


def _memory_event_matches(
    actual: Mapping[str, Any], expected: Mapping[str, Any]
) -> bool:
    """Match structured memory while allowing semantic content assertions."""
    content = str(
        actual.get("content") or actual.get("summary") or actual.get("value") or ""
    )
...
def score_expected(case: EvalCase, run: AgentRun) -> list[Score]:
    ...
    value, reason = _contains_all(
        run.response, phrases if isinstance(phrases, list) else [phrases]
    )
    scores.append(
        Score("response_contains", value, value >= 1.0, weight=1.0, reason=reason)
    )
    ...
    missing_tools = [
        ...
        if not any(_tool_matches(item, name) for name in called)
    ]
    tool_value = (
        1.0
        if not required_tools
        else (len(required_tools) - len(missing_tools)) / len(required_tools)
    )
    scores.append(
        Score(
            "required_tools",
            ...
            reason=(
                ...
                else f"missing tools: {missing_tools}"
            ),
        )
    )

    required_status = expected.get("required_tool_status", {})
    if isinstance(required_status, Mapping):
        failures: list[str] = []
        for expected_name, wanted_status in required_status.items():
            matches = [
                tool
                for tool in run.tools
                if _tool_matches(str(expected_name), tool.name)
            ]
            allowed = (
                {str(item) for item in wanted_status}
                if isinstance(wanted_status, (list, tuple, set, frozenset))
                else {str(wanted_status)}
            )
            if not matches or not any(tool.status in allowed for tool in matches):
                failures.append(f"{expected_name} status not in {sorted(allowed)}")
        ok = not failures
        scores.append(
            Score(
                "required_tool_status",
                ...
                hard=bool(expected.get("required_tool_status_hard", True)),
                reason=(
                    "all required tool statuses verified" if ok else "; ".join(failures)
                ),
            )
        )
    ...
    scores.append(
        Score(
            ...
            reason=(
                ...
                else f"forbidden tools: {used_forbidden}"
            ),
        )
    )
    ...
    if "memory_event" in expected:
        ...
        scores.append(
            Score(
                ...
                weight=2.0,
                hard=True,
                reason="memory event matched" if matched else "memory event not found",
            )
        )
    ...
    if "objective_outcome" in expected:
        wanted = str(expected["objective_outcome"])
        actual = run.objective_outcome
        ok = actual == wanted
        scores.append(
            Score(
                "objective_outcome",
                ...
                weight=3.0,
                hard=True,
                reason=f"objective_outcome={actual}, expected={wanted}",
            )
        )

    if "delivery_status" in expected:
        wanted = str(expected["delivery_status"])
        actual = run.delivery_status
        ok = actual == wanted
        scores.append(
            Score(
                "delivery_status",
                ...
                weight=2.0,
                hard=True,
                reason=f"delivery_status={actual}, expected={wanted}",
            )
        )
    ...
    if "max_latency_ms" in expected and run.latency_ms is not None:
        ...
        scores.append(
            Score(
                "latency",
                1.0 if ok else max(0.0, limit / run.latency_ms),
                ok,
                weight=0.5,
                reason=f"{run.latency_ms:.1f}ms <= {limit:.1f}ms",
            )
        )
    return scores
...
def score_trajectory(case: EvalCase, run: AgentRun) -> Score:
    ...
    retries_ok = (
        len(run.tools) <= int(policy["max_tool_calls"])
        if "max_tool_calls" in policy
        else True
    )
    ...
    return Score(
        ...
        ok,
        weight=1.5,
        hard=bool(policy.get("hard", False)),
        reason=reason,
    )
...
def score_rubric(
    ...
    *,
    ...
    judge_reasons = getattr(judged, "reasons", {})
    scores: list[Score] = []
    deterministic_scores = score_expected(case, run)
    deterministic_scores.extend(score_evidence(case, run))
    ...
    for criterion in rubric:
        ...
        source = (
            "rubric_judge" if criterion.evaluator == "llm" else "deterministic_rubric"
        )
        reason = (
            str(judge_reasons.get(criterion.criterion_id) or "judge score")
            if criterion.evaluator == "llm"
            else "deterministic check"
        )
        if criterion.evaluator == "deterministic" and criterion.check in deterministic:
            ...
            reason = fallback.reason
            source = "deterministic_rubric_fallback"
        if raw is None and criterion.evaluator == "llm":
            source = "unassessed"
            reason = "LLM criterion was not assessed"
...
def aggregate_scores(scores: list[Score]) -> tuple[float, bool]:
    ...
    reward = (
        sum(score.value * max(0.0, score.weight) for score in scores) / total_weight
    )
    return (0.0 if hard_fail else reward), not hard_fail and all(
        score.passed for score in scores if score.hard
    )
