...
def build_parser() -> argparse.ArgumentParser:
    ...
    run = sub.add_parser(
        "run", help="run a JSONL dataset with deterministic replay fixtures"
    )
    ...
    run.add_argument(
        "--analysis", default="", help="optional failure hotspot JSON path"
    )
    run.add_argument("--gate", choices=["acceptance", "hard"], default="acceptance")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "run":
        ...
        summary = EvalHarness(
            dataset_name=args.dataset_name, version=args.version
        ).run_sync(cases, ReplayExecutor())
        if args.store:
