...
def xiaoman_plugins_root(plugins_home: Path | None = None) -> Path:
    return plugins_home or (Path.home() / ".xiaoman-plugin")


def global_registry_path(plugins_home: Path | None = None) -> Path:
    return xiaoman_plugins_root(plugins_home) / "registry.json"
...
def remove_plugin_registry_entry(
    plugin_id: str,
    *,
    plugins_home: Path | None = None,
) -> bool:
