...
class ActivePluginInfo:
    ...
    module_path: str
    declares_xiaoman_plugin: bool = False
...
class PluginManager:
    ...
    def discover(self) -> list[dict[str, str]]:
        ...
        for source in resolve_plugin_sources(
            ...
            mods.append({
                ...
                "import_path": f"xiaoman_plugin_{import_source}_{import_suffix}",
    ...
    async def _load_one(self, mod: dict[str, str]) -> None:
        ...
        registered_tools_before = (
            self._tool_registry.get_registered_names()
            if self._tool_registry is not None
            else set()
        )
        unsubscribe_handlers: list[Callable[[], None]] = []
        ...
        if module_path:
            ...
            if cls is None:
                ...
                self._discard_imported_plugin(mp)
                return
            instance = cls()
            ...
            try:
                ...
                unsubscribe_handlers = self._bind_handlers(instance, mp)
                ...
                if hasattr(instance, "terminate"):
                    try:
                        ...
                        logger.exception(
                            "插件 %s 初始化回滚时 terminate 失败",
                            mod["name"],
                        )
                for unsubscribe in reversed(unsubscribe_handlers):
                    unsubscribe()
                self._discard_imported_plugin(mp)
                if self._tool_registry is not None:
                    added_tools = (
        ...
        self._active_plugins[mp] = ActivePluginInfo(
            ...
            manifest=manifest,
            module_path=mp,
            declares_xiaoman_plugin=descriptor is not None,
            skill_roots=_skill_roots_for_policy(plugin_policy, descriptor),
            drift_skill_roots=_drift_skill_roots_for_policy(plugin_policy, descriptor),
            mcp_servers=_mcp_servers_for_policy(plugin_policy, descriptor),
        )
    ...
    def _import_plugin(self, module_name: str, path: Path) -> None:
        ...
        try:
            ...
            self._discard_imported_plugin(module_name)
            raise

    @staticmethod
    def _discard_imported_plugin(module_name: str) -> None:
    ...
    def _bind_handlers(
        ...
        for md in plugin_registry.get_handlers_by_module_path(module_path):
            ...
            unsubscribers.append(self._event_bus.on(ctx_type, bound))
...
def _apply_manifest_fields(instance: Any, raw: dict[str, object]) -> None:
    for field_name in _MANIFEST_FIELDS:
        ...
        if val is not None:
            setattr(instance, field_name, str(val))
...
def _resolve_plugin_data_dir(
    ...
    if descriptor is None or not marketplace:
        return None
    return Path.home() / ".xiaoman-plugin" / "data" / f"{descriptor.name}-{marketplace}"
