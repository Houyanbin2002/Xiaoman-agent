...
from agent.plugins.xiaoman_descriptor import PluginDescriptor, load_plugin_descriptor
...
def run_plugin_doctor(
    *,
    ...
    resolved_workspace = workspace or (Path.home() / ".xiaoman" / "workspace")
    ...
    return {
        "status": (
            _merge_status([item["status"] for item in plugins])
            if plugins
            else "healthy"
        ),
        "plugins": plugins,
...
def _check_install(
    ...
    if descriptor is not None:
        manifest_name = (
            ...
            if not descriptor.is_legacy
            else "兼容旧版插件清单"
        )
        return _check("install", "ok", f"已发现 {manifest_name}: {plugin_root}")
...
def _check_lifecycle(entry: dict[str, object]) -> dict[str, str]:
    ...
    if not lifecycle_entry:
        return _check(
            "lifecycle", "error", "capabilities.lifecycle=true 但缺少 lifecycle_entry"
        )
    if not Path(lifecycle_entry).exists():
        return _check(
            "lifecycle", "error", f"lifecycle_entry 不存在: {lifecycle_entry}"
        )
    ...
    return _check(
        "lifecycle",
        "warn",
        "声明存在但当前 active=false，可能需要重启或被运行态 gating",
    )
...
def _check_skills(
    ...
    normal_missing = [
        ...
        if not is_plugin_skill_materialized(
            plugin_skill_materialization_path(workspace / "skills", name),
            logical_name=name,
        )
    ]
    ...
    drift_missing = [
        ...
        if not is_plugin_skill_materialized(
            ...
            logical_name=f"{base_name}:{name}",
        )
    ]
    expected = len(normal_skills) + len(drift_skills)
    if expected == 0:
        return _check(
            "skills", "warn", "capabilities.skills=true 但 registry 里没有 skill"
        )
    missing = normal_missing + [f"{base_name}:{name}" for name in drift_missing]
    if not missing:
        return _check("skills", "ok", f"共 {expected} 个 skill 已就位")
    return _check("skills", "warn", f"缺少 skill: {', '.join(missing)}")
...
def _check_mcp(
    ...
    if not server_names:
        return _check(
            "mcp", "error", "capabilities.mcp=true 但 registry 里没有 mcp_servers"
        )
    if descriptor is None:
        return _check(
            "mcp",
            "warn",
            f"registry 声明了 {len(server_names)} 个 MCP server，未校验 manifest",
        )
