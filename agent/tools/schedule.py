...
class ScheduleTool(Tool):
    ...
    def build_job(self, **kwargs: Any) -> ScheduledJob:
        ...
        if tier not in ("instant", "soft"):
            raise ValueError(f"tier 须为 instant 或 soft，收到 {tier!r}")
        if trigger not in ("at", "after", "every"):
            raise ValueError(f"trigger 须为 at/after/every，收到 {trigger!r}")
        if tier == "instant" and not message:
            raise ValueError("tier=instant 时 message 为必填项")
        if tier == "soft" and not prompt:
            raise ValueError("tier=soft 时 prompt 为必填项")
        if not channel or not chat_id:
            raise ValueError("channel 和 chat_id 为必填项")

        try:
            ZoneInfo(tz)
        except ZoneInfoNotFoundError:
            raise ValueError(f"无效的时区 {tz!r}") from None
...
class CancelScheduleTool(Tool):
    ...
    async def execute(self, **kwargs: Any) -> str:
        ...
        if job_id:
            all_ids = [job.id for job in self._service.list_jobs()]
            matches = [
