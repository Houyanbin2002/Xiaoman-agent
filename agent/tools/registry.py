...
class ToolRegistry:
    ...
    def __init__(self, backend: SearchBackend | None = None) -> None:
        ...
        self._context: ContextVar[dict[str, str]] = ContextVar(
            f"tool_registry_context_{id(self)}",
            default={},
        )
    ...
    def get_risk(self, name: str) -> str:
        """Return a tool's declared risk without exposing registry internals."""
        meta = self._metadata.get(name)
        return meta.risk if meta is not None else "external-side-effect"
    ...
    def get_deferred_names(
        ...
        格式按 source_type 分组；非 builtin 来源下再按 source_name 分组。
        """
        always_on = self.get_always_on_names()
        excluded = always_on | _META_TOOLS | (visible or set())
        builtin: list[str] = []
        sourced: dict[str, dict[str, list[str]]] = {}

        for name, doc in self._documents.items():
            if name in excluded:
                continue
            if doc.source_type != "builtin":
                sourced.setdefault(doc.source_type, {}).setdefault(
                    doc.source_name or doc.source_type,
                    [],
                ).append(name)
            else:
                builtin.append(name)
        ...
        for source_type, groups in sorted(sourced.items()):
            result[source_type] = {
                key: sorted(names) for key, names in sorted(groups.items())
            }
