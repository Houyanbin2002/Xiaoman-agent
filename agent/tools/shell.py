...
_BLOCK_DEFAULT_MS = 30_000  # process_output block 默认单次等待
_BLOCK_MAX_MS = 30_000  # process_output block 单次硬上限：超过按上限，迫使长任务走轮询而非一次死等
...
async def _bg_pump(
    ...
    with open(log_path, "wb") as f:
        ...
        try:
            ...
            # 取消 pump 时，嵌套的 stdout/stderr drain task 不会自动继承取消。
            # 必须在文件句柄关闭前显式收拢，否则它们稍后读到数据时会继续写已关闭文件。
...
class ShellTool(Tool):
    ...
    def description(self) -> str:
        shell_name = "Windows 命令提示符（cmd.exe）" if _IS_WINDOWS else "POSIX shell"
        chaining = "多条命令用 && 连接" if _IS_WINDOWS else "多条命令用 ; 或 && 连接"
        return (
            f"在{shell_name}中执行命令并返回输出。\n"
            "注意：\n"
            "- 使用绝对路径，避免依赖 cd 切换目录\n"
            f"- {chaining}，不要用换行分隔\n"
        ...
        return {
            ...
            "properties": {
                "command": {
                    "type": "string",
                    "description": (
                        "要执行的 Windows cmd.exe 命令"
                        if _IS_WINDOWS
                        else "要执行的 POSIX shell 命令"
                    ),
                },
                "description": {
                    "type": "string",
                ...
                "run_in_background": {
                    ...
                    "description": (
                        ...
                        "输出写入日志文件，通过 process_output 获取、process_stop 停止。"
    ...
    async def _execute_background(
        ...
        log_fd, log_path = tempfile.mkstemp(
            prefix=f"xiaoman-bg-{task_id}-", suffix=".log"
        )
        os.close(log_fd)
    ...
    async def _execute_with_auto_promote(
        ...
        task_id = f"shell_{uuid4().hex[:12]}"
        log_fd, log_path = tempfile.mkstemp(
            prefix=f"xiaoman-fg-{task_id}-", suffix=".log"
        )
        os.close(log_fd)
...
class ShellTaskOutputTool(Tool):
    ...
    def description(self) -> str:
        return (
            "读取后台 shell 进程的当前输出和状态。它不属于任务中心。\n"
            "返回字段：\n"
            "- status: 'running' | 'done'\n"
            ...
            "收到 process_output 结果后，结合你对命令的了解，判断这个进程是否应该继续运行：\n"
            "需要 process_stop 终止的情况（有输出不代表不该 stop）：\n"
            "  - 进程是死循环或明确没有退出条件（无论是否产生输出）\n"
            "  - 进程挂起：有过输出但 since_last_output_ms 异常大，不符合该命令的预期节奏\n"
            "  - 进程卡死：从未有过输出（since_last_output_ms=null）且 elapsed_ms 超过合理预期\n"
            "不需要 stop 的情况：编译、下载、训练等明确会结束的长时间任务，或用户主动要求的服务进程。\n"
            "- 这是轮询接口：block=true 最多等待 timeout_ms（上限 30s）就返回一次快照，不会一直阻塞到任务结束。\n"
            "- 长进程用轮询循环：每次 block 一段→返回后判断 done/继续轮询/process_stop，不要期望一次调用就等到结束（传超大 timeout_ms 也会被钳到 30s）。\n"
            "- 分次轮询才能在等待期间响应用户新消息或 /stop；一次死等会让你长时间无响应。\n"
            "- 如果你决定放弃某个后台进程、准备输出最终回复，必须先调用 process_stop 将其终止，再生成回复。不要让后台进程在你回复后继续孤立运行。\n"
...
class ShellTaskStopTool(Tool):
    ...
    @property
    def description(self) -> str:
        return "停止后台 shell 进程（SIGKILL 整棵进程树）并从注册表移除。它不属于任务中心。"
...
def _write_full_output(content: str) -> str:
    fd, path = tempfile.mkstemp(prefix="xiaoman-shell-", suffix=".log")
    os.close(fd)
