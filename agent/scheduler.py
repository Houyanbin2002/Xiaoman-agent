...
class JobStore:
    ...
    def _from_dict(self, d: dict[str, Any]) -> ScheduledJob:
        ...
        if d.get("last_attempt_at"):
            d["last_attempt_at"] = self._parse_dt(d["last_attempt_at"])
...
class SchedulerService:
    ...
    def __init__(
        ...
        _now_fn: Callable[[], datetime] | None = None,
        event_publisher: Callable[[ScheduledJobChanged], None] | None = None,
    ) -> None:
    ...
    def add_job(self, job: ScheduledJob) -> None:
        # Semantic batches can create exact reminders during runtime bootstrap,
        # before SchedulerService.run() starts. Load persisted jobs first so a
        # pre-start add never overwrites the existing schedule file.
        if not self._loaded:
            self.load_and_recover()
        ...
        self._publish_job_change("upserted", job)
        logger.info(
            f"Job added: {job.id[:8]} tier={job.tier} trigger={job.trigger} "
            f"fire_at={job.fire_at.isoformat()}"
        )

    def replace_job(self, job_id: str, replacement: ScheduledJob) -> bool:
        current = self._jobs.get(job_id)
        if current is None:
            return False
        replacement.id = current.id
        replacement.created_at = current.created_at
        replacement.run_count = current.run_count
        replacement.enabled = True
        replacement.last_attempt_at = current.last_attempt_at
        replacement.last_status = "pending"
        replacement.last_error = None
        self._jobs[job_id] = replacement
        self.store.save(self._jobs)
        self._publish_job_change("upserted", replacement)
        return True
    ...
    def cancel_job(self, job_id: str) -> bool:
        ...
        self.store.save(self._jobs)
        self._publish_job_change("cancelled", job)
        return True
    ...
    def cancel_job_by_name(self, name: str) -> list[str]:
        ...
        jobs = [self._jobs[jid] for jid in cancelled]
    ...
    def load_and_recover(self) -> None:
        ...
        for job in jobs:
            if not job.enabled:
                # Keep failed one-shot jobs visible so users can inspect or edit them.
                self._jobs[job.id] = job
                count_loaded += 1
                continue
            ...
            if job.fire_at <= now:
                ...
                if job.trigger == "every":
                    ...
                    self._publish_job_change("expired", job)
            else:
                self._jobs[job.id] = job
                count_loaded += 1
    ...
    async def _execute_and_reschedule(self, job: ScheduledJob) -> None:
        succeeded = False
        job.last_attempt_at = self._now()
        try:
            await self._execute(job)
            job.run_count += 1
            job.last_status = "sent"
            job.last_error = None
            succeeded = True
        except Exception as e:
            ...
            # A cancellation that happened while the job was executing must win.
            ...
            if job.trigger == "every":
                ...
                # Preserve a failed one-shot job for diagnosis/editing without
                # retrying it every scheduler tick.
                job.enabled = False
                self._jobs[job.id] = job
            else:
                self._jobs.pop(job.id, None)
                self._publish_job_change("fired", job)
            self.store.save(self._jobs)

    def _publish_job_change(self, action: str, job: ScheduledJob) -> None:
        if self._event_publisher is None:
            return
        payload = asdict(job)
        for key in ("fire_at", "created_at", "last_attempt_at"):
            value = payload.get(key)
            if isinstance(value, datetime):
                payload[key] = value.isoformat()
        self._event_publisher(
            ScheduledJobChanged(action=action, job_id=job.id, job=payload)
        )

    async def _execute(self, job: ScheduledJob) -> None:
        label = job.name or job.id[:8]
        if job.tier == "instant":
            result = await self._push_message(job, job.message or "")
            ...
            if content:
                result = await self._push_message(job, content)
                logger.info(f"[scheduler] soft 推送完成 {label!r}: {result}")
            else:
                raise RuntimeError(f"soft AI 返回空内容：{label}")

    async def _push_message(self, job: ScheduledJob, message: str) -> str:
        typed_sender = getattr(type(self.push_tool), "send", None)
        if callable(typed_sender):
            delivery = await self.push_tool.send(
                channel=job.channel,
                chat_id=job.chat_id,
                message=message,
            )
            if not bool(getattr(delivery, "success", False)):
                raise RuntimeError(str(getattr(delivery, "message", "消息发送失败")))
            return str(getattr(delivery, "message", "文本已发送"))
        ...
        if result.startswith(("发送失败", "错误：", "渠道 ")) or "没有可用的 sender" in result:
