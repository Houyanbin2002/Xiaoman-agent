...
class ProactiveTurnPipelineDeps:
    ...
    priority_signal_fn: Callable[[datetime], bool | Awaitable[bool]] | None = None
...
class ProactiveTurnPipeline:
    ...
    def __init__(self, deps: ProactiveTurnPipelineDeps) -> None:
        ...
        self._gate_chain = ProactiveGateChain(
            ...
            priority_signal_fn=deps.priority_signal_fn,
            rng=self._rng,
        )
        ...
        if (
            ...
            and self._drift_pipeline.step_recorder is None
        ):
    ...
    async def run(self, frame: ProactiveFrame) -> ProactiveFrame:
        ...
        with diagnostic_context(
            session=self._session_key, flow="proactive", tick=ctx.tick_id
        ):
            logger.info(
                diagnostic_line(
                    "ProactiveTurnPipeline.run",
    ...
    async def _run_with_context(
        self, ctx: AgentTickContext, started: float
    ) -> float | None:
        gate = await self._gate_check(ctx)
        if gate.blocked:
            logger.info(
                diagnostic_line(
                    "ProactiveTurnPipeline.run",
    ...
    async def _gate_check(self, ctx: AgentTickContext) -> GateResult:
        gate = await self._gate_chain.check(ctx)
        if gate.blocked:
            return gate
        if gate.reason == "priority_signal":
            return gate
        ...
        if self._rng.random() < float(pass_probability):
            return gate
        reason = str(
            self._proactive_slots.get("proactive:gate:reason") or "plugin_gate"
        )
    ...
    def _collect_proactive_plugin_state(self) -> None:
        ...
        self._proactive_effect_logs = [
            ...
            if key.startswith("proactive:effect:") and isinstance(value, dict)
        ]
    ...
    async def _fetch_pull(self, ctx: AgentTickContext) -> FeedResult:
        ...
        if (
            not gw_result.alerts
            and not gw_result.content_meta
            ...
            if self._drift_pipeline is not None and self._cfg.drift_enabled:
                ...
                min_interval_hours = max(
                    0, int(self._cfg.drift_min_interval_hours or 0)
                )
                if (
    ...
    async def _judge_evaluate(
        self, ctx: AgentTickContext, messages: list[dict]
    ) -> None:
    ...
    async def _deliver_execute(
        self, ctx: AgentTickContext, decision: ResolveResult
    ) -> float | None:
