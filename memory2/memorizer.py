...
class Memorizer:
        ...
        if memory_type in ("procedure", "preference"):
            ...
            if memory_type == "procedure":
                ...
                if merge_target is not None:
                    ...
                    result = f"merged:{merge_target['id']}"
                    self._register_execution_state(
                        result,
                        memory_type=memory_type,
                        extra=extra,
    ...
    def _register_execution_state(
        ...
        *,
        ...
        if memory_type != "procedure":
            return
        item_id = str(write_result or "").split(":", 1)[-1].strip()
        if not item_id:
            return
        existing = self._store.execution.get(item_id)
        desired = build_execution_state(
            item_id=item_id,
            metadata=extra,
            source_ref=source_ref,
            verified=verified,
        ...
        if existing is None:
            self._store.execution.upsert(desired)
            return
        authority = (
            ExecutionAuthority.USER
            if ExecutionAuthority.USER in {existing.authority, desired.authority}
            else (
                desired.authority
                if desired.authority is not ExecutionAuthority.LEARNED
                else existing.authority
            )
        )
        lifecycle = existing.lifecycle_status
        if desired.lifecycle_status is ExecutionLifecycleStatus.ACTIVE:
            lifecycle = ExecutionLifecycleStatus.ACTIVE
        if (
            verified
            or authority is not existing.authority
            or lifecycle is not existing.lifecycle_status
            or desired.extraction_confidence > existing.extraction_confidence
            or source_ref not in existing.evidence_refs
        ):
            self._store.execution.upsert(
                replace(
                    existing,
                    verification_status=(
                    ...
                    user_locked=existing.user_locked or desired.user_locked,
                    extraction_confidence=max(
                        existing.extraction_confidence,
                        desired.extraction_confidence,
                    ),
                    last_verified_at=(
                    ...
                    evidence_refs=tuple(
                        dict.fromkeys((*existing.evidence_refs, source_ref))
                    ),
                )
            )
