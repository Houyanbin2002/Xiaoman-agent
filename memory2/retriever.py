...
class Retriever:
    ...
    def __init__(
        ...
        embed_timeout_s: float = _DEFAULT_EMBED_TIMEOUT_S,
    ) -> None:
        self._store = store
        ...
        self._embed_timeout_s = max(0.05, float(embed_timeout_s))
    ...
    async def _retrieve_vector_lanes(
        ...
        *,
        ...
        actual_hotness_alpha = (
            ...
            else max(0.0, min(1.0, float(hotness_alpha)))
        )
        try:
            hit_groups = self._store.vector_search_batch(
                ...
                scope_chat_id=scope_chat_id,
                require_scope_match=require_scope_match,
                hotness_alpha=actual_hotness_alpha,
