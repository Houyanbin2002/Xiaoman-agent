...
把 Xiaoman 插件理解成一个独立 Git 仓库。

这个仓库可以给 Xiaoman 提供三类能力：
...
你想改的是“Xiaoman 自己怎么工作”，就写 lifecycle。
...
一个仓库一个插件仍是最简单的发布方式，适合单一能力：

组织地址：

- <https://github.com/orgs/xiaoman-plugins/repositories>
...
当一个组织要维护多款插件时，仓库可以作为**插件市场**。市场只是一个可浏览的目录；添加市场不会安装其中任何插件。

```text
市场仓库
├─ .xiaoman-plugin/marketplace.json  <- 可安装插件清单
└─ plugins/
   ├─ feed/
   │  └─ .xiaoman-plugin/plugin.json
   └─ calendar/
      └─ .xiaoman-plugin/plugin.json
```
...
市场项的 `source` 可以是市场仓库内以 `./` 开头的目录，也可以是独立 Git 来源：
...
```bash
python main.py plugin-marketplace-add --source owner/my-tools
python main.py plugin-marketplace-list
python main.py plugin-marketplace-install --marketplace my-tools --plugin feed
python main.py plugin-marketplace-update my-tools
```

Dashboard 的“插件”页提供同样的添加、刷新、浏览和安装操作。无论来自市场还是直接 Git 安装，最终都会进入同一份本地版本缓存；安装后重启 Gateway 才会加载新能力。

旧版插件清单会在安装时兼容读取；更早的只有 `plugin.py` 的插件也会从静态元数据生成一份**仅位于本地缓存**的兼容清单，原仓库不会被修改。只有 `SKILL.md` 的开放技能包也可以通过 `--subdir` 导入，安装器会把它包装成 skills-only 插件。新插件和市场只能发布 `.xiaoman-plugin` 格式。

卸载会清理插件代码和注册表，但默认保留插件数据；CLI 加上 `--purge-data` 或 Dashboard 勾选“同时删除插件数据”才会清理数据目录。
...
## 7. 最关键的文件：`.xiaoman-plugin/plugin.json`
...
```json
{
  "name": "my-skills",
  "version": "0.1.0",
  "description": "A bundle of Xiaoman skills",
  "paths": {
    "skills": ["skills"]
  },
  "xiaoman": {
...
```json
{
  "name": "my-plugin",
  "version": "0.1.0",
  "description": "Full Xiaoman plugin",
  "paths": {
    "skills": ["skills"],
...
```text
~/.xiaoman/workspace/skills/hello-world
```

如果这个 skill 目录出现并能被 `plugin-doctor` 识别，你就已经打通第一步了。
...
- `workspace/skills` 下有没有可用的 skill 物化目录
...
```text
发布前检查
├─ 1. 仓库里有 .xiaoman-plugin/plugin.json
...
```text
~/.xiaoman-plugin/data/<plugin>-<marketplace>/
```
...
`xiaoman-plugins` 现在是组织，不是机器可消费的中心 index。
...
│  └─ 主仓 plugins/ 下的内建插件
└─ installed
   └─ ~/.xiaoman-plugin/cache/<marketplace>/<plugin>/<version>/
```

外加一个“组织级入口”：

```text
GitHub 上的 xiaoman-plugins/*
```
...
第一，外部 `.xiaoman-plugin` 插件现在也能稳定拿到 `data_dir`，所以插件自己的迁移逻辑应该写在插件内，而不是写进主仓。

第二，plugin 不只是“被动回复里的一个 patch”，它现在也能碰到：

```text
PluginContext 可支撑的扩展面
├─ workspace skill 物化目录
├─ 独立 MCP server
├─ session / memory 协作
└─ 插件自己的持久化状态
```
...
skill 不复制进主仓；运行时优先创建软链接，系统不允许创建软链接时会
降级为带管理标记的受管副本：

```text
active plugins
├─ 读取 manifest 里的 skill roots
├─ 计算逻辑 skill name 和跨平台物理目录名
├─ 物化到 ~/.xiaoman/workspace/skills
└─ drift skill 物化到 ~/.xiaoman/workspace/drift/skills
```

普通 `.xiaoman-plugin` skill 会用裸名。

drift skill 会带插件名前缀，避免撞名。

#### 14.7.1 跨平台物化规则

skill 统一由运行时维护。Unix/macOS 通常使用软链接；Windows 没有符号
链接权限时使用受管副本，并在插件内容变化时按指纹刷新。用户自己的同名
目录不会被删除或覆盖。

```text
skill link rules
├─ 普通 .xiaoman-plugin skills
│  └─ ~/.xiaoman/workspace/skills/<skill-name>
├─ drift skills
│  └─ 逻辑名 <plugin>:<skill-name>（物理目录会编码 Windows 非法字符）
└─ 非 .xiaoman-plugin 的旧 builtin
   └─ 仍可能带 plugin_id 前缀
```
...
`~/.xiaoman-plugin/registry.json` 是本机插件总表。
