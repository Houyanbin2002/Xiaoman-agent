...
def _phase_qqbot(a: WizardAnswers) -> None:
    ...
    _hint("使用腾讯开放平台 WebSocket 长连接，与 Telegram、企业微信独立并存")
...
def _render_channels(a: WizardAnswers) -> str:
    ...
    lines += [
        "# 个人微信使用 Dashboard「网关与频道」里的扫码登录；Token 存入系统凭据库",
        "[plugins.weixin]",
        "enabled = true",
        'account_id = ""',
        "allow_from = []",
        ...
        "",
        "# 企业微信官方智能机器人（管理后台选择 API 模式 → 使用长连接）",
        "# [plugins.wecom]",
        "# enabled = true",
        '# bot_id = ""',
        '# secret = ""',
        '# allow_from = []  # 企业微信 userid；留空允许所有可见成员',
        "",
    ]
...
def _print_completion(a: WizardAnswers) -> None:
    ...
    if a.qqbot_app_id and not a.qqbot_user_openid:
        click.echo()
        _warn("QQBot allow_from 为空，将允许所有能联系机器人的用户")
        _hint("如需限制使用者，向 bot 发一条消息后把 user_openid 填入 config.toml：")
        _hint("[plugins.qqbot]")
        _hint('allow_from = ["<user_openid>"]')
