...
class EventBus:
    ...
    def bind_running_loop(self) -> None:
        """Bind background delivery to the app loop for thread-safe producers."""
        loop = asyncio.get_running_loop()
        if self._loop is not None and self._loop is not loop:
            raise RuntimeError("event bus is already bound to another event loop")
        self._loop = loop
        self._ensure_observe_queue()
    ...
    def off(
        ...
        """Remove one exact handler registration; safe to call repeatedly."""
    ...
    async def emit_durable(
        ...
        *,
        ...
        """Deliver observers independently and expose stable per-handler receipts.

        A retried durable event skips handlers whose receipt was already stored,
        so one failing memory domain does not replay successful writes in others.
        """

        failures: list[str] = []
        handlers = self._handlers.get(cast(type[object], type(event)), [])
        occurrences: dict[str, int] = {}
        for raw_handler in handlers:
            name = _handler_name(raw_handler)
            ordinal = occurrences.get(name, 0)
            occurrences[name] = ordinal + 1
            # The ordinal is local to handlers with the same name. Adding an
            # unrelated consumer therefore cannot invalidate stored receipts.
            consumer_id = f"{name}#{ordinal}"
            if consumer_id in delivered:
                continue
            handler = cast(Handler[E], raw_handler)
            try:
                result = handler(event)
                ...
                if result is not None:
                    event = cast(E, result)
                on_success(consumer_id)
            except Exception:
                failures.append(consumer_id)
                logger.exception(
                    "durable observer error for %s handler=%s",
                    type(event).__name__,
                    consumer_id,
                )
        if failures:
            raise RuntimeError(
                f"durable event delivery failed for {len(failures)} consumer(s)"
            )
        return event
    ...
    def enqueue(
        ...
        if running_loop is None:
            if self._loop is None or self._loop.is_closed():
                raise RuntimeError(
                    "event bus has not been bound to an application loop"
                )
            self._loop.call_soon_threadsafe(self.enqueue, event)
            return
