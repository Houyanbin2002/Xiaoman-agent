...
async def ack_post_guard_fail(
    ...
    *,
    ...
    # Delivery/message guards mean no alert was actually delivered. Alerts stay
    # pending so a later tick can re-evaluate them; only content classifications
    # receive a short ACK here.
...
class ProactiveResolver:
    ...
    async def resolve(self, ctx: AgentTickContext) -> ResolveResult:
        ...
        blocked_reason = self._blocked_alert_reason(ctx)
        if blocked_reason:
            ctx.terminal_action = "skip"
            ctx.skip_reason = "user_busy"
            ctx.skip_note = f"个人注意力边界阻止投递：{blocked_reason}"
            return self._build_skip_result(ctx)
    ...
    @staticmethod
    def _blocked_alert_reason(ctx: AgentTickContext) -> str:
        cited = set(ctx.cited_item_ids)
        for item in ctx.fetched_alerts:
            key = f"{item.get('ack_server', '')}:{item.get('event_id') or item.get('id', '')}"
            if key in cited and item.get("delivery_allowed") is False:
                return str(item.get("delivery_block_reason") or "notification_policy")
        return ""
