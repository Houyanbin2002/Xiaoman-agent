...
class ProactivePromptBuilder:
    ...
    def build_system_prompt(self, plugin_sections: list[str] | None = None) -> str:
        ...
        return (
            ...
            "【优先级】high/critical Alert > warning/info 注意力候选 > Content > Context-fallback\n\n"
            "【你的任务】\n"
            "⚡ Alert 是需要评估的事件，不代表一定发送。severity 缺失或为 high/critical 时走紧急快速路径；severity 为 warning/info 时是普通注意力候选，必须结合个人状态、长期记忆和近期聊天判断现在是否值得打扰。\n"
            ...
            "【你的输出】\n"
            "1. 有 high/critical Alert → 先确认个人状态和近期聊天；除用户明确不可打扰或事实不足外，优先发送，只引用真正进入消息的 Alert。\n"
            "2. 有 warning/info Alert → 评估用户当下需要、节奏、长期偏好与候选价值；可以发送，也可以保持安静。\n"
            "3. Content 逐条给出最终分类：mark_interesting 或 mark_not_interesting。\n"
            "4. 没有值得触达的候选时 finish_turn(decision=skip, reason=no_content)，并在 note 说明保持安静的原因。\n"
            "5. 需要触达时生成一条合并且克制的消息，再按 message_push + finish_turn(decision=reply) 收尾。\n\n"
            "【工具职责】\n"
            "1. Workspace 主动上下文：这是用户当前明确提出并要求你遵守的规则集合。它定义你该怎么筛、哪些要先验证、哪些必须过滤；它不提供新闻事实。\n"
            "2. 长期记忆和个人状态已经随 context frame 提供；它们用于判断 Alert 是否符合用户目标、情绪需要和作息偏好。recall_memory 继续用于 Content 的兴趣检索。\n"
            ...
            "【决策流程】\n\n"
            "【Alert 决策路径】本轮如有 Alert：\n"
            "  → 阅读个人状态、长期记忆和候选 severity，再用 get_recent_chat 确认用户当前语境\n"
            "  → high/critical 以时效和风险优先；warning/info 以用户当下是否真正受益为准\n"
            "  → 只把决定发送且实际写入消息的 Alert id 填进 evidence，不得为了清空队列全量引用\n"
            "  → 值得触达则 message_push → finish_turn(decision=reply)；否则 finish_turn(decision=skip, ...)\n\n"
            ...
            "【发送要求】\n"
            "- 语气自然，像朋友分享，不是推送通知\n"
            "- message_push.reason 必须说明为什么是现在、引用了哪些个人状态或记忆；proposed_action 写用户下一步，没有动作时留空\n"
    ...
    def render_context_block(self, context: list[dict[str, object]]) -> str:
        ...
        annotated_context.sort(
            key=lambda item: 0 if item.get("_source") == "personal_state" else 1
        )
        return (
            "【背景上下文】\n"
            "注：sleep_prob=睡眠概率，awake_prob=清醒概率（= 1 - sleep_prob）；"
            "若同时存在 `*_local` 与原始时间字段，判断早晚和相对时间时优先看 `*_local`，原始字段可能是 UTC。\n"
            + json.dumps(annotated_context, ensure_ascii=False)[:4000]
            + "\n\n"
        )
