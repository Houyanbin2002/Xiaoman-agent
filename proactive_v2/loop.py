...
from core.tracing import new_trace_id, record_trace_event, trace_root
...
"""
    ...
    def _build_agent_tick(self):
        ...
        self._agent_tick_deps = AgentTickDeps(
            cfg=self._cfg,
            sense=self._sense,
            presence=self._presence,
            ...
            personal_source=self._personal_source,
        )
        return AgentTickFactory(self._agent_tick_deps).build()

    def reconfigure_model(self, *, provider: LLMProvider, model: str) -> None:
        """Switch proactive generation for later ticks without restarting its loop."""

        self._provider = provider
        self._model = model
        deps = getattr(self, "_agent_tick_deps", None)
        if deps is not None:
    ...
    async def _run_loop(self) -> None:
        ...
        while self._running:
            ...
            if not self._running:
                break
            try:
                last_base_score = await self.run_event_tick()
            except Exception:
                logger.exception("ProactiveLoop tick 异常")
                last_base_score = None

    async def _wait_for_tick(self, interval: int) -> None:
        wake_event = getattr(self, "_wake_event", None)
        if wake_event is None:
            await asyncio.sleep(interval)
            return
        try:
            await asyncio.wait_for(wake_event.wait(), timeout=max(1, interval))
        except TimeoutError:
            pass
        wake_event.clear()
    ...
    async def run_event_tick(self) -> float | None:
    ...
    async def _tick(self) -> float | None:
        ...
        with diagnostic_context(session=session_key, flow="proactive", phase="tick"):
            with trace_root(
                getattr(self, "_trace_recorder", None),
                trace_id=new_trace_id(),
                flow="proactive",
                session_key=session_key,
                title="主动巡查",
                metadata={"channel": self._cfg.default_channel},
            ):
                logger.info(
                    diagnostic_line(
                        "ProactiveLoop._tick",
