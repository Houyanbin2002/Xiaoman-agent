...
class AgentTickFactory:
    ...
    def build(self) -> ProactiveTurnPipeline:
        ...
        source = self._build_attention_source()
        ...
        return ProactiveTurnPipeline(
            ProactiveTurnPipelineDeps(
                ...
                priority_signal_fn=(
                    source.has_priority_signal
                    if hasattr(source, "has_priority_signal")
                    else None
                ),
            )
        )
    ...
    def _build_attention_source(self) -> AttentionGatewaySource:
        return AttentionGatewaySource(
            content_limit=self._deps.cfg.agent_tick_content_limit,
            personal_source=self._deps.personal_source,
        )
    ...
    def _build_tool_deps(self, source: AttentionGatewaySource) -> ToolDeps:
        web_fetch_tool = None
        try:
