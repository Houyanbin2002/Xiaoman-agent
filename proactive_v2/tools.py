    ...
    _schema(
        ...
        ),
        {
            ...
            "properties": {
                ...
                "reason": {
                    "type": "string",
                    "description": "为什么现在值得联系用户；必须结合候选事件、个人状态、记忆或近期语境说明",
                },
                "proposed_action": {
                    "type": "string",
                    "description": "希望用户采取的下一步；只是信息分享时可为空",
                },
            },
...
async def _recall_memory(ctx: AgentTickContext, args: dict, *, memory) -> str:
    ...
    hits = await _retrieve_interest_hits(
        memory=memory, query=query, timestamp=ctx.now_utc
    )
    if not hits:
...
def commit_notification_draft(ctx: AgentTickContext) -> str:
    """Commit a staged proactive draft into the common Resolve/Deliver phases."""
    if not ctx.has_notification_draft:
        raise ValueError("notification draft is empty")
    ctx.final_media = list(ctx.draft_media)
...
def _finish_turn(ctx: AgentTickContext, args: dict) -> str:
    ...
    if decision == "reply":
        ...
        return commit_notification_draft(ctx)
    if decision == "skip":
        if ctx.has_notification_draft:
            raise ValueError(
...
def _message_push(ctx: AgentTickContext, args: dict) -> str:
    ...
    ctx.draft_reason = str(args.get("reason") or "").strip()
    ctx.draft_action = str(args.get("proposed_action") or "").strip()
    return json.dumps({"ok": True}, ensure_ascii=False)
