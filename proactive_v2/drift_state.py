...
class DriftStateStore:
            ...
            result.append(
                {
                    ...
                    "run_id": (
                        int(row["run_id"] or 0) if row["run_id"] is not None else None
                    ),
    ...
    def load_briefing(self, skills: list[SkillMeta]) -> str:
        ...
        for skill in skills[:8]:
            ...
            if status == "completed":
                lines.append(
                    "  前情：已闭环；内部续航便签只在选中该 skill 后参考，不作为待办。"
                )
            else:
    ...
    def skill_dir_for(self, skill_name: str) -> Path | None:
        ...
        if not name:
            return None
        materialized_dir = plugin_skill_materialization_path(self.skills_dir, name)
        workspace_candidates = [materialized_dir]
        legacy_dir = self.skills_dir / name
        if legacy_dir != materialized_dir:
            workspace_candidates.append(legacy_dir)
    ...
    def save_finish(
        ...
        *,
        ...
        with self._connection() as conn:
            ...
            old_cursor = (
                self._decode_json_object(row["cursor_json"]) if row is not None else {}
            )
    ...
    def append_step(
        ...
        *,
        ...
        created_at = now_utc.isoformat()
        run_id = (
            self._last_saved_run_id if created_at == self._last_saved_run_at else None
        )
        with self._connection() as conn:
            _ = conn.execute(
                """
    ...
    def _normalize_status(raw: Any) -> str:
        ...
        return (
            status if status in {"idle", "completed", "paused", "waiting"} else "idle"
        )
    ...
    def _load_skill_meta(self, skill_dir: Path, *, builtin: bool) -> SkillMeta | None:
        ...
        expected_name = (
            skill_dir.name if builtin else plugin_skill_logical_name(skill_dir)
        )
        if not name or not description or name != expected_name:
            logger.info(
        ...
        last_run_at = parse_iso(continuum.get("last_run_at")) or parse_iso(
            raw_state.get("last_run_at")
        )
