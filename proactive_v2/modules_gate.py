...
class ProactiveGateChain:
    def __init__(
        ...
        *,
        ...
        priority_signal_fn: Callable[[datetime], bool | Awaitable[bool]] | None,
        rng: Any,
    ) -> None:
        self._cfg = cfg
        ...
        self._priority_signal_fn = priority_signal_fn
        self._rng = rng

    async def check(self, ctx: AgentTickContext) -> GateResult:
        ...
        if self._priority_signal_fn is not None:
            try:
                priority = self._priority_signal_fn(ctx.now_utc)
                if inspect.isawaitable(priority):
                    priority = await priority
                if priority:
                    return GateResult(
                        blocked=False,
                        reason="priority_signal",
                        base_score=None,
                        context_as_fallback_open=False,
                    )
            except Exception as exc:
                logger.warning("[proactive_v2] priority signal probe failed: %s", exc)
